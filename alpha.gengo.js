 ...
 * @link        https://github.com/mashpie/gengo-node
 * @license     http://opensource.org/licenses/MIT
 *
 * @version     0.4.1
 */
//Going to play with this code until I can figure it out.
...
var vsprintf = require('sprintf').vsprintf,
    ...
    url = require('url'),
    path = require('path'),
    debug = require('debug')('gengo:debug'),
    warn = require('debug')('gengo:warn'),
    error = require('debug')('gengo:error'),
...
gengo.configure = function gengoConfigure(opt) {
  ...
  directory = (typeof opt.directory === 'string') ? __dirname + pathsep + opt.directory : __dirname + pathsep + 'locales';
...
gengo.init = function gengoInit(request, response, next) {
...
gengo.__ = function gengoTranslate(phrase) {
...
gengo.__n = function gengoTranslatePlural(singular, plural, count) {
  var msg, namedValues, args = [];
...
gengo.setLocale = function gengoSetLocale(locale_or_request, locale) {
  ...
  return gengo.getLocale(request);
};

gengo.getLocale = function gengoGetLocale(request) {
...
gengo.getCatalog = function gengoGetCatalog(locale_or_request, locale) {
  var target_locale = locale_or_request;
...
gengo.overrideLocaleFromQuery = function (req) {
  if (req === null) {
    return;
  }
  ...
  if (urlObj.query.locale) {
    ...
    gengo.setLocale(req, urlObj.query.locale.toLowerCase());
  }
};
...
function guessLanguage(request) {
  if (typeof request === 'object') {
    ...
    gengo.setLocale(request, request.language);
  }
}
