...
(function() {
    ...
    utils.debug = function(value) {
        ...
        function exists(level) {
            var result;
            _.each(configure().debug().level, function(res) {
                if (level === res) {
                    result = true;
                }
            });
            ...
            info: function(message) {
                switch (typeof configure().debug()) {
                    case 'boolean':
                        ...
                        break;
                    case 'object':
                        if (utils.Object(configure().debug().level).size() > 1) {
                            if (exists('info')) {
                                cout(value, message, 'info');
                            }

                        } else {
                            if (configure().debug().level === 'info') {
                                cout(value, message, 'info');
                            }
                        }
                        break;
                ...
                switch (typeof configure().debug()) {
                    ...
                    case 'object':
                        if (utils.Object(configure().debug().level).size() > 1) {
                            ...
                            if (configure().debug().level === 'error') {
                                console.log();
                                cout(value, message, 'error');
                            }
                        }
                ...
                switch (typeof configure().debug()) {
                    ...
                    case 'object':
                        if (utils.Object(configure().debug().level).size() > 1) {
                            ...
                            if (configure().debug().level === 'debug') {
                                console.log();
                                cout(value, message, 'debug');
                            }
                        }
